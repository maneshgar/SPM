...
void File_Manager::export_obj(std::vector<cv::Mat> point_cloud, std::string fileName){
    std::cout << "Exporting to Obj file...\n";
    std::cout << "Number of points :: " << point_cloud.size() << std::endl;
    std::ofstream of(this->root + fileName, std::ofstream::out);
    for (uint i=0; i<point_cloud.size(); i++){
        ...
        of << std::fixed << std::setprecision(8) << "v " << point_cloud[i].at<float>(0,0) <<"\t"<<point_cloud[i].at<float>(1,0)<<"\t"<<point_cloud[i].at<float>(2,0)<<std::endl;
    }
    //    of.close();
}
...
void File_Manager::export_Mat32FC3(cv::Mat data, std::string name){
    ...
    for (uint i = 0; i < data.rows; i++){
        for (int j = 0; j < data.cols; ++j) {
            of << std::fixed << std::setprecision(8) << data.at<cv::Vec3f>(i,j)[0] << "\t" << data.at<cv::Vec3f>(i,j)[1] <<" \t "<< data.at<cv::Vec3f>(i,j)[2] <<std::endl;
        }
    }
}

void File_Manager::export_Mat32FC3_normal(cv::Mat data, std::string name){
    std::cout << "Exporting " + name + "...\n";
    std::ofstream of(this->root + name + ".data", std::ofstream::out);
    of << data.rows << std::endl;
    of << data.cols << std::endl;
    for (uint i = 0; i < data.rows; i++){
        for (int j = 0; j < data.cols; ++j) {
            of << std::fixed << std::setprecision(8) << data.at<cv::Vec3f>(i,j)[0] << "\t" << data.at<cv::Vec3f>(i,j)[2] <<" \t "<< data.at<cv::Vec3f>(i,j)[1] <<std::endl;
        }
    }
}
