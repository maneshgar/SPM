...
Image *ImageProcessing::holeFill2(Image *input_image, int max_neighbourhood_size, int min_neighbourhood_size, bool dominant_neighbour)	{
    ...
    int count = 0;
    while (have_holes && count < 40)	{
        std::printf("Step %i started.\n", count);
        count++;
        have_holes = false;
...
std::string PBRT_Shape_Str(Color P0, Color P1, Color P2, Color P3, Color P4, int id){
    /*
    ...
    std::string str = "";
    if(true){
        str += "NamedMaterial \"m" + std::to_string(id) + "\"\n";
    }
    str += "\nShape \"trianglemesh\" \"integer indices\" [0 1 2 2 3 4]\n";
    str += "\"point P\" [";
    str += std::to_string(P0.r()) + " " + std::to_string(P0.g()) + " " + std::to_string(P0.b()) + "   ";
    str += std::to_string(P1.r()) + " " + std::to_string(P1.g()) + " " + std::to_string(P1.b()) + "   ";
    str += std::to_string(P2.r()) + " " + std::to_string(P2.g()) + " " + std::to_string(P2.b()) + "   ";
    str += std::to_string(P3.r()) + " " + std::to_string(P3.g()) + " " + std::to_string(P3.b()) + "   ";
    str += std::to_string(P4.r()) + " " + std::to_string(P4.g()) + " " + std::to_string(P4.b()) + "   ";
    str += "] \n";
    return str;
}

std::string PBRT_Material_str(int id){
    /*
      * MakeNamedMaterial "m3" "string type" "fourier" "string bsdffile" "bsdfs/m3.bsdf"
      */
    std::string path = "bsdfs/";
    std::string name = "m" + std::to_string(id);
    std::string str = "MakeNamedMaterial \"" + name + "\" \"string type\" \"fourier\" \"string bsdffile\" \"" + path + name + ".bsdf\"\n";
    return str;
}

bool ImageProcessing::ExportToPBRT(Image image){

    cv::Mat mask(image.getHeight(), image.getWidth(), CV_32FC1, cv::Scalar(0.0));

    int Id = 0;
    Color P0,P1,P2,P3,P4;
    std::string shapes_str = "";
    std::string material_str = "";
    std::string shapes_tmp, material_tmp;
    for (int rowIt = 1; rowIt < image.getHeight()-1; ++rowIt) {
        //        std::cout << "Row :++++++++++++++: " << rowIt << std::endl;
        for (int colIt = 1; colIt < image.getWidth()-1; ++colIt) {


            P0 = image.getPixel(colIt,rowIt-1);
            P1 = image.getPixel(colIt-1,rowIt);
            P2 = image.getPixel(colIt,rowIt);
            P3 = image.getPixel(colIt+1,rowIt);
            P4 = image.getPixel(colIt,rowIt+1);
            if (P0 == Color (0.0, 0.0, 0.0) ||
                    P1 == Color (0.0, 0.0, 0.0) ||
                    P2 == Color (0.0, 0.0, 0.0) ||
                    P3 == Color (0.0, 0.0, 0.0) ||
                    P4 == Color (0.0, 0.0, 0.0)
                    ) {
                mask.at<float>(rowIt, colIt) = 0.0;
            }else{
                Id = rowIt * image.getHeight() + colIt;
                material_tmp = PBRT_Material_str(rowIt/100);
                shapes_tmp = PBRT_Shape_Str(P0, P1, P2, P3, P4, rowIt/100);
                //                std::cout << "COL " << colIt << "\n" << str;
                shapes_str += shapes_tmp + "\n";
                material_str += material_tmp + "\n";
                mask.at<float>(rowIt, colIt) = 1.0;
            }
        }
    }
    ...
    File_Manager fm("/home/behnam/MyDrive/Workstation/Qt/build-Compensation-Desktop_Qt_5_6_0_GCC_64bit-Debug/");
    fm.write_to_file("shapes.pbrt", shapes_str);
    fm.write_to_file("materials.pbrt", material_str);
    std::cout << "PBRT Files are ready to use.\n";
}
