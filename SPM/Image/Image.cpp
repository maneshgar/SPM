...
Image::Image(cv::Mat img){
    ...
    for (int y=0;y<height;y++)	{
        data[y] = new Color[width];
        for (int x=0;x<width;x++)	{
            cv::Vec3f temp = img.at<cv::Vec3f>(y,x);
            data[y][x] = Color(temp[0], temp[1], temp[2], 1);
        }
    }
...
void Image::split(std::vector<Image> &channels)	{

    Image* B = new Image(getWidth(), getHeight());
    Image* G = new Image(getWidth(), getHeight());
    Image* R = new Image(getWidth(), getHeight());
    ...
    for (int y=0;y<height;y++)	{
        for (int x=0;x<width;x++)	{
            std::cout << B->getPixel(x, y);
        }
    }

    B = ImageProcessing::bilateralFilteringGrayscale(B);
    G = ImageProcessing::bilateralFilteringGrayscale(G);
    R = ImageProcessing::bilateralFilteringGrayscale(R);

    return;
}

void Image::merge(std::vector<Image> channels, Image &outImage)	{

    for (int y=0;y<channels[0].height;y++)	{
        for (int x=0;x<channels[0].width;x++)	{
            float val[3];
            val[0] = channels[0].getPixelGrayscale(x,y);
            val[1] = channels[1].getPixelGrayscale(x,y);
            val[2] = channels[2].getPixelGrayscale(x,y);
            Color temp_val(val[0], val[1], val[2]);
            outImage.setPixel(x,y,temp_val);
        }
    }
    return;
}
...
//			if (denom < TC_EPSILON)    denom = 1.0f;/////////////////////////////////Behnam it is not comment
...
void Image::getcvMat(cv::Mat &outputMat){
    cv::Mat temp(this->getHeight(), this->getWidth(), CV_32FC3);
    for (int i = 0; i < getHeight(); ++i) {
        for (int j = 0; j < getWidth(); ++j) {
            cv::Vec3f temp_vec;
            temp_vec[0] = this->getPixel(j, i).b();
            temp_vec[1] = this->getPixel(j, i).g();
            temp_vec[2] = this->getPixel(j, i).r();
            temp.at<cv::Vec3f> (i,j) = temp_vec;
        }
    }
...
void Image::getcvMat_RGB(cv::Mat &outputMat){
    cv::Mat temp(this->getHeight(), this->getWidth(), CV_32FC3);
    for (int i = 0; i < getHeight(); ++i) {
        for (int j = 0; j < getWidth(); ++j) {
            cv::Vec3f temp_vec;
            temp_vec[0] = this->getPixel(j, i).r();
            temp_vec[1] = this->getPixel(j, i).g();
            temp_vec[2] = this->getPixel(j, i).b();
            temp.at<cv::Vec3f> (i,j) = temp_vec;
        }
    }
    outputMat = temp.clone();
...
void Image::getcvMat_GRB(cv::Mat &outputMat){
    cv::Mat temp(this->getHeight(), this->getWidth(), CV_32FC3);
    for (int i = 0; i < getHeight(); ++i) {
        for (int j = 0; j < getWidth(); ++j) {
            cv::Vec3f temp_vec;
            temp_vec[0] = this->getPixel(j, i).r();
            temp_vec[1] = this->getPixel(j, i).b();
            temp_vec[2] = this->getPixel(j, i).g();
            temp.at<cv::Vec3f> (i,j) = temp_vec;
        }
    }
    outputMat = temp.clone();
