...
class Image	{
	public:
        ...
        void getcvMat_RGB(cv::Mat &outputMat);
        void getcvMat_GRB(cv::Mat &outputMat);
        void getcvMat(cv::Mat &outputMat);

		///Sets the pixel value at the specified location
		void setPixel(int x, int y, Color const &color);

        ///Split the image to three BGR channels
        void split(std::vector<Image> &channels);

        ///Merges a vector of three channels to one image
        static void merge(std::vector<Image> channels, Image &outImage);
